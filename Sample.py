...
class Server:
    ...
    def proxy_thread(self, client_connection_socket, client_address):
        ...
        client_request = client_connection_socket.recv(1024).decode()
    ...
    def find_file(self, url_file_name, client_connection_socket, port_number, client_address, start_time, url_connect, url_slash):
        try:
            ...
            client_connection_socket.send(response_message.encode())
            end_time = time.time()
            ...
            try:
                ...
                web_request = str()
                if url_slash:
                    web_request = b"GET /".decode() + url_slash[1:] + " HTTP/1.1\nHost: " + url_connect + "\r\n"
                else:
                    web_request = b"GET / HTTP/1.1\nHost: ".decode() + url_connect + "\r\n"

                # print "GET web request: "+web_request
                proxy_connection_socket.send(web_request.encode())
                ...
                while True:
                    ...
                    if len(web_server_response) > 0:
                        web_server_response_append += web_server_response.decode()
                    else:
                ...
                if timeout_flag:
                    ...
                    client_connection_socket.send(web_server_response_append.encode())
                end_time = time.time()
                ...
                proxy_temp_file.write(response_to_file.encode())
